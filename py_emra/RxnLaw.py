...
def RxnLaw(S,Params,NumParams,RxnTypes,X,Rev,i):
	...
	if rtype in (7,9): Metabs.append(symbols('Outer'))#Outer is at the end
...
def One_to_2_Rev(X,K):
	...
	return Vmax*(Sub-Prod1*Prod2/Keq)*(1/Km1)/(1+Sub1/Km1+Prod1/Km2+Prod2/Km3+Prod1*Prod2/Km2/Km3)

def Two_to_1_Rev(X,K):
	Sub1=X[0]; Sub2=X[1]; Prod1=X[2]; Vmax=K[0]; Keq=K[1]; Km1=K[2]; Km2=K[3]; Km3=K[4];
	return Vmax*(Sub*Sub2-Prod1/Keq)*(1/Km2/Km3)/(1+Sub1/Km1+Sub2/Km2+Sub1*Sub2/Km1/Km2+Prod1/Km3)

def Two_to_2_Rev(X,K):
	Sub1=X[0]; Sub2=X[1]; Prod1=X[2]; Prod2=X[3]; Vmax=K[0]; Keq=K[1]; Km1=K[2]; Km2=K[3]; Km3=K[4]; Km4=K[5];
	N = Vmax*(Sub*Sub2-Prod1/Keq)*(1/Km2/Km3)
